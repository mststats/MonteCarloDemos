...
MH_mixture=function(x, M, iter, prop.paras=c(1,1,1/20), init.z=kmeans(x,M)$cluster) {
  ...
  mu[1,]=replicate(M, function(m) mean(x[z[1,]==m]))
  sigma[1,]=replicate(M, function(m) sd(x[z[1,]==m]))
  
  log.q=function(t.mu, t.sigma, t.z)
    sum(replicate(n, function(j) dnorm(x[j], t.mu[t.z[j]], t.sigma[t.z[j]], log=TRUE))) - sum(log(t.sigma))
  log.J=function(p.sigma, c.sigma)
    sum(replicate(M, function(m) dgamma(p.sigma[m],(c.sigma[m]^2)/prop.paras[2],rate=c.sigma[m]/prop.paras[2],log=TRUE)))
  ...
  for (i in 2:iter) {
    ...
    for (j in 1:n) {
      ...
      probs=replicate(M, function(m) dnorm(x[j],mu[i-1,m],sigma[i-1,m]))
      probs=probs/sum(probs) # normalisation
      z[i,j]=sample(1:M,1,prob=probs) 
    }
    mu.star=mu[i-1,]+rnorm(M, 0, prop.paras[1])
    sigma.star=replicate(M, function(m) rgamma(1,(sigma[i-1,m]^2)/prop.paras[2],rate=sigma[i-1,m]/prop.paras[2]))
