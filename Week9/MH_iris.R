...
MH_mixture=function(x, M, iter, prop.paras=c(1e-2,4e-3), init.z=kmeans(x,M)$cluster) {
  accept=0
  n=length(x)
  ...
  for (i in 2:iter) {
    ...
    log.r=log.q(mu.star, sigma.star, z[i,])-log.J(sigma.star, sigma[i-1,])-
          log.q(mu[i-1,], sigma[i-1,], z[i,])+log.J(sigma[i-1,], sigma.star)
